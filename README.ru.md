...
Slizen — самостоятельно развёртываемый одноузловой RESP-прокси. Он выявляет
горячие ключи, помещает их в ограниченный локальный кэш и объединяет
одновременные запросы при промахе кэша. Redis или Valkey остаётся источником
истины.

[![В нагрузке skew-99/1 версии v0.2.3-rc.1 Slizen снизил число физических GET к источнику с 94 961 до 759](.github/assets/slizen-origin-relief.gif)](https://github.com/slizendb/slizen/releases/tag/v0.2.3-rc.1)

*v0.2.3-rc.1, skew-99/1: 94 961 прямой физический `GET` к источнику против 759
через Slizen на 100 000 операций (на 99,2% меньше), без ошибок и расхождений.
Прямой p99 был ниже: 1,53 мс против 2,05 мс.
[Методика](docs/BENCHMARKING.md) ·
[Данные релиза](https://github.com/slizendb/slizen/releases/tag/v0.2.3-rc.1)*

**Предварительная версия для разработчиков.** Slizen подходит, когда снижение
нагрузки на источник важнее дополнительной задержки прокси. Он не предназначен
для Redis Cluster, аварийного переключения через Sentinel, широкой
совместимости с командами Redis и нагрузок, в которых прямые записи в источник
должны становиться видимыми немедленно.
