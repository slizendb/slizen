...
**Evidence, а не обещание скорости.** В воспроизводимом image-bound synthetic-run v0.2.1 на 1 000 ключей с распределением 99/1 Slizen измерил **на 88,734% меньше origin GET на успешное чтение** при нуле request failures, value mismatches и final-validation mismatches. При этом proxy p99 был `2,856 ms` против `1,777 ms` напрямую, поэтому это не утверждение «Slizen быстрее». Вот [сырой JSON релиза](https://github.com/slizendb/slizen/releases/download/v0.2.1/slizen-workload-result.json) и [методика](docs/BENCHMARKING.md); результат относится только к конкретному runner, конфигурации и workload.
