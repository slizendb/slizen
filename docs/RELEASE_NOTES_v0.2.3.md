...
- There are no new configuration fields. Existing v0.2.2 observe-first and per-prefix policy files remain valid.
...
| --- | ---: |
| Direct origin GETs | 94,961 in every run |
...
Every repeat reached the request limit with zero request failures, value mismatches, final-validation failures, and final-validation mismatches. The p99 ranges do not show a speed win over direct local Valkey. This is not a speed claim: the defensible result is approximately 99.15% fewer origin GETs for this exact skewed workload on this host, not “Slizen makes Redis faster,” a production capacity number, or a universal 99% guarantee.
...
- Confirm the stable 99/1 scenario includes the fixed miss attribution and positive origin reduction. The gate intentionally has no latency or universal 99% threshold.
